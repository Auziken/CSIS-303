...
string Infix::convertToPostfix(string infix){
    // :(
    stack <char> S; // Initalize our stack, S, because I don't want to type a long name 200 times.
    string postfix = ""; // Here's our final product. 
    char ch; // And this will grab each part of the string that we iterate through.
    ...
    for(int i = 0; i<infix.length(); i++){
        ...
        if(ch == ' '){ // If our current part of the infix statement is blank, skip it.
        ...
        else if(isOperand(ch)){ // If it's an operand, add it to the postfix statement immediately. Operands mostly come BEFORE the operators in postfix.
            postfix += ch;
        }
        else if (isOperator(ch)){ // If it's an operator...
            while(!S.empty() && greaterOrEqual(S.top(), ch)){ // While our stack ISN'T empty, and this operator has a lesser priority to the top of the stack...
            ...
            while (!S.empty() && S.top() != '('){ // While our stack ISN'T empty, AND the top of our stack ISN'T an open parenthesis...
                postfix += S.top(); // Add the top of our stack to the postfix. This serves as our cleanup.
                S.pop(); // And pop the top.
            }
            S.pop(); // As well as popping the final part of the stack, as that'll be the end of our postfix statement.
        }
    }
    return postfix; // Return the abomination.
}
